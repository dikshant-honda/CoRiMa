...
class Trajectory:
    ...
    ***** add following functionality *****
    ...
    path_prediction = f(path, v_lon)

    path = LineString((x1, y1), (x2, y2), ..., (xn, yn))
    p_lon = v_lon * time

    trajectory = path.interpolate(p_lon)

    uncertain_traj = Uncertain_trajectories(trajectory)

    risk_model(uncertain_traj)
    '''

    def path(self) -> NDArray[np.float64]:
        """List of waypoints to follow"""
        waypoint_samples = [Point(-0.9, -10.0), Point(-0.9, -6.0), Point(-0.9, -2.0), Point(-0.1, -0.1), Point(2.0, 0.9), Point(6.0, 0.9), Point(10.0, 0.9)]
